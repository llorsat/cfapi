...
function remove_spiner(container) {
  var container = $(container);
...
/**
 * Specific awesome interaction functions for next h5
 */

function addSpin(selector, options) {
  selector = $(selector);
  if(!options) {
    options = {};
  }
  ...
  if($widget.is('button') || $widget.is('a')) {
    ...
    spinerize_button(selector, options);
  } else if($widget.is('select')) {
    loading_combobox(selector);
  } else if($widget.is('input')) {
    spinerize_input(selector);
  }
}

function removeSpin(selector, character) {
  var $widget = $(selector);
  ...
  if($widget.is('button')) {
    despinerize_button(selector, $widget.data('prev-text'));
  } else if($widget.is('select')) {
    finished_combobox(selector);
  } else if($widget.is('input')) {
    despinerize_input(selector, character);
  }
}
...
function mini_spinner(element){
  var selector_width = element.width();
  var selector_height = element.height();
  var opts = {
    lines: 9,
    length: 3,
    width: 3,
  ...
  var pixels_top = selector_height * 11 / 100;
  $(spinner.el).css('width', selector_width + 'px');
  $(spinner.el).css('height', selector_height + 'px');
  $(spinner.el).css('left', (selector_width / 2) - 2 + 'px');
  $(spinner.el).css('top', (selector_height / 2) - pixels_top + 'px');
...
/*
 * Create a generic vitrine with generic conf object:
 * vitrineId, vitrineTitle
 */

function spinerize_vitrine(vitrineId){
  var vitrine = $("#" + vitrineId + ".vitrine ");
  var buttons = vitrine.find(".vitrine-buttons");
  var spinnerButton = $("<div>", {
    'class': "vitrine-button vitrine-spinner"
  }).css({
    'right': "90px",
    'top': '17px',
    "cursor": "none"
  });
  var spinner = mini_spinner(spinnerButton);
  spinnerButton.append(spinner.el);
  buttons.prepend(spinnerButton);
}

function despinerize_vitrine(vitrineId){
  var vitrine = $("#" + vitrineId + ".vitrine ");
  var buttons = vitrine.find(".vitrine-buttons");
  var spinnerButton = buttons.find(".vitrine-spinner");
  spinnerButton.remove();
}

//TODO: Unir código a las funciones spinerize y despinerize

function spinerize_button(selector, options) {
...
function despinerize_button(target, value) {
  target = $(target);
  if (!value) {
...
function spinerize_input(selector) {
  var selector = $(selector);
...
function despinerize_input(target, value) {
...
function loading_combobox(selector) {
  var selector = $(selector);
  var option = $('<option>', {
    'text': __('Cargando...'),
  })
...
function finished_combobox(selector) {
  var selector = $(selector);
  selector.html('');
